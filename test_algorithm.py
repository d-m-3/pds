...
def pds_k_regular_bipartite_algorithm(BG):
    ...
    # UNFINISHED.
    # Set of neighbors of u, v_X, v_Y.
    set_neigh = set(list(BG.neighbors(u)) + list(BG.neighbors(v_X)) 
                    + list(BG.neighbors(v_Y)))
    set_neigh.difference_update(pds_bar)
    print(set_neigh)
    #Should only work for 12 vertices for the moment.
    for v4 in set_neigh:
        neighbors_v4 = set(list(BG.neighbors(v4)))
        neighbors_v4.difference_update(pds_bar)
        print("N(v4)", v4, neighbors_v4)
        if all(v in pds_list for v in neighbors_v4):
            pds_list.remove(v4)
            pds_bar.append(v4)
            break
    return pds_list
    ...
    for s in pds_list:
        # Stop if the PDS has the maximum size.
