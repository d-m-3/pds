...
**Goal:** Test the following conjecture computationally "Every cubic graph *G = (V, E)* on *n = |V|* vertices with *n > 8*, has at least one PDS *G[S]* of maximum possible size such that *d_S(u) = 2* for every vertex *u* in *S*". We proved that this conjecture was false by finding counterexamples.\
